...
function ($, LivefyreContent) {
    ...
    describe('A LivefyreContent object', function () {
        ...
        describe('.hasRightsGranted', function () {
            it('is true if the rights granted status is set', function () {
                var c = new LivefyreContent();
                c.rights = {status: 'granted'};
                expect(c.hasRightsGranted()).toBe(true);
            });

            it('is false if not set or not granted', function () {
                var c = new LivefyreContent();
                expect(c.hasRightsGranted()).toBe(false);

                c = new LivefyreContent();
                c.rights = {status: 'expired'};
                expect(c.hasRightsGranted()).toBe(false);
            });
        });

        describe('.hasProducts', function () {
            it('is true if there are products', function () {
                var c = new LivefyreContent();
                c.links = {product: [{}]};
                expect(c.hasProducts()).toBe(true);
            });

            it('is false if there are no products', function () {
                var c = new LivefyreContent();
